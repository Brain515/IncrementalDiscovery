...
data = [['BPI2012A_Algorithm', 'Time', 'Memory', 'Precision', 'Fitness', 'F-value']]
...
def alpha_flow():
    ...
    log_folder = "D:\\BPI-Month\\BPIC_2012_A\\Adding_Logs_For_Traditional_Discovery"
    logs = []
...
def IPDF_alpha_flow():
    ...
    start_mem = show_info()  # 开始内存
    start = time.time()  # 开始时间
    log_folder = r"D:\BPI-Month\BPIC_2012_A"
    logs = []
    for file_name in os.listdir(log_folder):
        if file_name.endswith(".xes"):
            ...
            log = xes_importer.apply(log_path)
            logs.append(log)
    dfg1 = dfg_discovery.apply(logs[0])
    for i in range(len(logs)):
        if i >= 1:
            if i != len(logs) - 2:
                dfg = dfg_discovery.apply(logs[i])
                dfg1.update(dfg)
            elif i == len(logs) - 2:
                dfg = dfg_discovery.apply(logs[i])
                dfg1.update(dfg)
                ...
                dfg_visualization.view(gviz)  # 第n-1个有向图 47.7KB
    ...
    log = xes_importer.apply(
        os.path.join("D:\BPI-Month\BPIC_2012_A", "Adding_Logs_For_Traditional_Discovery", "12For10-3.xes"))
...
def IPDF_alpha_flow():
    ...
    start_mem = show_info()  # 开始内存
    start = time.time()  # 开始时间
    log_folder = "D:\\BPI-Month\\BPIC_2012_A"
    logs = []
    dfg1 = None
