...
data = [['BPI20154-Algorithm', 'Time', 'Memory', 'Precision', 'Fitness', 'F-value']]
...
def alpha_flow():
    ...
    log_folder = "D:\\BPI-Month\\BPIC_2015_4\\Adding_Logs_For_Traditional_Discovery"
    logs = []
...
def IPDF_alpha_flow():
    ...
    start_mem = show_info()  # 开始内存
    start = time.time()  # 开始时间
    log_folder = "D:\\BPI-Month\\BPIC_2015_4"
    logs = []
    dfg1 = None
    ...
    log = xes_importer.apply(
        os.path.join("D:\BPI-Month\BPIC_2015_4", "Adding_Logs_For_Traditional_Discovery", "15For10-3.xes"))
...
def IPDF_IMd_flow():
    ...
    # end = time.time()  # 结束时间
    # end_mem = show_info()  # 结束内存
    # print("程序运行时间：%.6f秒" % (end - start))  # 27.41
    # print(f'一共占用{end_mem - start_mem}MB')  # 467
    # log = xes_importer.apply(
    #     os.path.join("D:\BPI-Month\BPIC_2015_4", "Adding_Logs_For_Traditional_Discovery", "15For10-3.xes"))
    # prec = precision_evaluator.apply(log, net, im, fm, variant=precision_evaluator.Variants.ETCONFORMANCE_TOKEN)
