...
csv_file = r'C:\Users\Administrator\Desktop\output.csv'
data = [['BPI20153-Algorithm', 'Time', 'Memory', 'Precision', 'Fitness', 'F-value']]
...
def alpha_flow():
    ...
    log_folder = "D:\\BPI-Month\\BPIC_2015_3\\Adding_Logs_For_Traditional_Discovery"
    logs = []
    ...
    print("程序运行时间：%.2f秒" % (end - start))  # 82.8
    print(f'一共占用{end_mem - start_mem}MB')  # 2773
    last_log = logs[-1]
...
def IMd_flow():
    ...
    end_mem = show_info()  # 结束内存
    print("程序运行时间：%.4f秒" % (end - start))  # 24.93
    print(f'一共占用{end_mem - start_mem}MB')  # 513
    last_log = logs[-1]
    prec = precision_evaluator.apply(last_log, net, im, fm, variant=precision_evaluator.Variants.ETCONFORMANCE_TOKEN)
    fitness = replay_fitness_evaluator.apply(last_log, net, im, fm, variant=replay_fitness_evaluator.Variants.TOKEN_BASED)
...
def IPDF_alpha_flow():
    ...
    start_mem = show_info()  # 开始内存
    start = time.time()  # 开始时间
    log_folder = "D:\\BPI-Month\\BPIC_2015_3"
    logs = []
    dfg1 = None
    ...
    log = xes_importer.apply(
        os.path.join("D:\BPI-Month\BPIC_2015_3", "Adding_Logs_For_Traditional_Discovery", "15For10-3.xes"))
...
def IPDF_IMd_flow():
    ...
    # end = time.time()  # 结束时间
    # end_mem = show_info()  # 结束内存
    # print("程序运行时间：%.4f秒" % (end - start))  # 27.41
    # print(f'一共占用{end_mem - start_mem}MB')  # 467
    # log = xes_importer.apply(
    #     os.path.join("D:\BPI-Month\BPIC_2015_3", "Adding_Logs_For_Traditional_Discovery", "15For10-3.xes"))
    # prec = precision_evaluator.apply(log, net, im, fm, variant=precision_evaluator.Variants.ETCONFORMANCE_TOKEN)
